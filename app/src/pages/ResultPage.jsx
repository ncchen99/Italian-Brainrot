...
export default function ResultPage() {
  ...
  const rewardItem = location.state?.reward ?? { imageSrc: ingredientImages.holyTomato, nameKey: 'dashboard.ingredients.i2_title', name: '神聖番茄' };
      ...
      <div className="relative z-10 w-full max-w-sm flex flex-col items-center text-center">
        ...
        {isSuccess ? (
          <>
             ...
             <p className="text-gray-300 font-bold mb-12">{t('result.successDesc')}</p>
             ...
             <div className={`w-48 aspect-square rounded-[2rem] bg-gradient-to-br from-[#1A1D2E] to-[#2D314A] border-4 border-[#FBBF24] shadow-[0_0_30px_rgba(251,191,36,0.5)] flex flex-col items-center justify-center transform transition-all duration-1000 ${showReward ? 'scale-100 rotate-0 opacity-100' : 'scale-50 rotate-[360deg] opacity-0'}`}>
                <img src={rewardItem.imageSrc || ingredientImages.holyTomato} alt={rewardItem.nameKey ? t(rewardItem.nameKey) : rewardItem.name} className="w-20 h-20 mb-4 object-contain drop-shadow-xl" />
                <div className="text-[#FBBF24] font-bold text-lg">{rewardItem.nameKey ? t(rewardItem.nameKey) : rewardItem.name}</div>
             </div>
          ...
          <>
             ...
             <div className="w-48 h-48 bg-gray-900 rounded-full border-4 border-gray-700 flex items-center justify-center mb-6 grayscale opacity-50 p-5">
               <img src={uiImages.burgerVillain} alt={t('result.villainImageAlt')} className="w-full h-full object-contain" />
             </div>

             <div className="bg-[#4c0519] border border-[#be185d] p-4 rounded-xl text-[#F472B6] font-bold mb-8 w-full shadow-inner">
               {t('result.villainQuote')}
             </div>
